...
class RdfService {
  ...
  */
  async getArray(term, uri = this.session.webId) {
    try {
  ...
  * @return {Promise<Array<NamedNode>>} A promise containing the array of NamedNode objects
  */
  async getFriends(uri = this.session.webId) {
  ...
  * @return {Promise<NamedNode>} A promise containing the NamedNode object
  */
  async getName(uri = this.session.webId) {
  ...
  * @return {Promise<NamedNode>} A promise containing the NamedNode object
   */
  async getPicture(uri = this.session.webId) {
  ...
  */
  async getInboxUrl(webId = this.session.webId) {
  ...
  * @return {Promise<Array<NamedNode>>} A promise containing the array of NamedNode objects
  */
  async getInboxContents(webId = this.session.webId) {
   ...
   */
  getChatFileUriForDate(year, month, day, chatUri) {
   ...
   */
  async getMessageUrisForFile(chatFileUri, chatUri) {
  ...
  */
  async getMessageMaker(msgUri, chatFileUri) {
  ...
  */
  async getMessageDate(msgUri, chatFileUri) {
    const subject = this.store.sym(msgUri);
    const chatFile = this.store.sym(chatFileUri);
  ...
  */
  async getMessageContent(msgUri, chatFileUri) {
    const subject = this.store.sym(msgUri);
    const chatFile = this.store.sym(chatFileUri);
    await this.fetcher.load(chatFile.doc());
  ...
  */
  async getChannelUri(myWebId, otherWebId) {
  ...
  */
  async appendMessage(chatFileUri, message) {
  ...
  * @return {Promise}
  */
  async deleteMessage(chatFileUri, message) {
   ...
   */
  buildMsgUri(chatFileUri, timeSent) {
  ...
  */
  async addListener(chatFileUri, callback) {
    const cFile = this.store.sym(chatFileUri);
  ...
  */
  async createNewChat(myWebId, otherWebIds, chatName) {
  ...
  */
  async createChatFileForDay(uri) {
  ...
  */
   ...
   */
  async sendNotifNewConv(webId, chatFolderUri, otherWebId, chatName, otherWebIds) {
  ...
  async sendNotifsNewMessage(otherWebIds, chatFolderUri, messageUri) {
    otherWebIds.forEach(e => {
   ...
   */
  async sendNotifNewMessage(webId, chatFolderUri, messageUri) {
  ...
  async sendNotifsDeletedMessage(otherWebIds, chatFolderUri, messageUri) {
    otherWebIds.forEach(e => {
   ...
   * @param {string} messageUri URI of the message the wbId is being notified of
   */
  async sendNotifDeletedMessage(webId, chatFolderUri, messageUri) {
    const inboxUrl = (await this.getInboxUrl(webId)).value;
    const notiFile = this.store.sym(inboxUrl + 'dechatnotifications.ttl');
    const messageUriFile = this.store.sym(messageUri);
    const chatFolderFile = this.store.sym(chatFolderUri);
   ...
   */
  async pushNotification(inboxUrl, ins) {
   ...
   */
  async setPermissions(resourceUri, webIds, ownerWebId) {
   ...
   */
  async deleteNotification(notificationUri) {
  ...
  async addChatToCard(myWebId, otherWebIds, chatFolder) {
    const myCardFile = this.store.sym(myWebId.replace('#me', '#'));
    const chatFolderFile = this.store.sym(chatFolder);
  ...
  testingMethod(myWebId, otherWebIds, chatName) {
    /*
  ...
  */
  async getConversations(myWebId) {
    const ret = new Array();
    ...
    await coinc.forEach(async element => {
      ...
      ret.push({
        chatFileUri: element.subject.value,
        others: participants.map(e => e.object.value),
      });
    });
    return ret;
  }

  async getConversationTitle(convUri) {
